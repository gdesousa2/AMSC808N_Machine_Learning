...
%% set up optimization problem - SVM with soft margins - Guilherme
[n,dim] = size(XX);
Y = (label*ones(1,dim + 1)).*[XX,ones(n,1)];
AW = [Y, eye(n); zeros(n,dim+1), eye(n)];
bSM = [ones(n,1); zeros(n,1)];
C = 100;
G = [eye(dim), zeros(dim,n+1); zeros(n+1,dim), zeros(n+1,n+1)];
Cvec = [zeros(dim+1,1); ones(n,1)];
Was = []; % active set

%gSM = @(x)(G*x + Cvec);
HSM = @(x)G;
x0 = FindInitGuess([w; zeros(n,1)],AW,bSM); % initial guess
[wSM1,~] = ASM(x0,@(x)(G*x + C*Cvec),HSM,AW,bSM,Was);

fprintf('wSM1 = [%d,%d,%d], b = %d\n',wSM1(1),wSM1(2),wSM1(3),wSM1(4));

% plotting
plane = wSM1(1)*xx+wSM1(2)*yy+wSM1(3)*zz+wSM1(4);
p = patch(isosurface(xx,yy,zz,plane,0));
p.FaceColor = '#FFA500';
p.EdgeColor = 'none';
camlight 
...
%%
function H = Hfun0(I,Y,w,lam)
aux = exp(-Y(I,:)*w);
d1 = size(Y,2);
Hv = sum(Y(I,:).*((aux.*(Y(I,:))./((1+aux).^2)).*ones(1,d1)),1)' + lam;
end
