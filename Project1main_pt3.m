...
lighting gouraud
alpha(0.3);
%% set up optimization problem - SINewton batch size - Guilherme
[n,dim] = size(XX);
lam = 0.01;
Y = (label*ones(1,dim + 1)).*[XX,ones(n,1)];
w0 = [-1;-1;1;1];
...
[w1,f1,gnorm1] = SINewton(fun,gfun,Hvec,Y,w0,100);
[w2,f2,gnorm2] = SINewton(fun,gfun,Hvec,Y,w0,200);
[w3,f3,gnorm3] = SINewton(fun,gfun,Hvec,Y,w0,400);
[w4,f4,gnorm4] = SINewton(fun,gfun,Hvec,Y,w0,800);

fprintf('w1 = [%d,%d,%d], b = %d\n',w1(1),w1(2),w1(3),w1(4));
fprintf('w2 = [%d,%d,%d], b = %d\n',w2(1),w2(2),w2(3),w2(4));
fprintf('w3 = [%d,%d,%d], b = %d\n',w3(1),w3(2),w3(3),w3(4));
fprintf('w4 = [%d,%d,%d], b = %d\n',w4(1),w4(2),w4(3),w4(4));
...
plane = w4(1)*xx+w4(2)*yy+w4(3)*zz+w4(4);
p = patch(isosurface(xx,yy,zz,plane,0));
p.FaceColor = 'green';
p.EdgeColor = 'none';
camlight 
...
grid;
niter = length(f);
plot((0:niter-1)',f,'Linewidth',2,'DisplayName','Bsz = 64');
plot((0:niter-1)',f1,'Linewidth',2,'DisplayName','Bsz = 100');
plot((0:niter-1)',f2,'Linewidth',2,'DisplayName','Bsz = 200');
plot((0:niter-1)',f3,'Linewidth',2,'DisplayName','Bsz = 400');
plot((0:niter-1)',f4,'Linewidth',2,'DisplayName','Bsz = 800');
set(gca,'Fontsize',fsz);
...
grid;
niter = length(gnorm);
plot((0:niter-1)',gnorm,'Linewidth',2,'DisplayName','Bsz = 64');
plot((0:niter-1)',gnorm1,'Linewidth',2,'DisplayName','Bsz = 100');
plot((0:niter-1)',gnorm2,'Linewidth',2,'DisplayName','Bsz = 200');
plot((0:niter-1)',gnorm3,'Linewidth',2,'DisplayName','Bsz = 400');
plot((0:niter-1)',gnorm4,'Linewidth',2,'DisplayName','Bsz = 800');
set(gca,'Fontsize',fsz);
set(gca,'YScale','log');
xlabel('k','Fontsize',fsz);
